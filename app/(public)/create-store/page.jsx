    ...
    try {
      ...
      if (["approved", "rejected", "pending"].includes(data.status)) {
        ...
        switch (data.status) {
          case "approved":
            setMessage(
              "Your store has been approved by an admin, you can now start adding products to your store from the dashboard",
            );
            setTimeout(() => router.push("/store"), 5000);
            break;
          case "rejected":
            setMessage(
              "Your store has been rejected by an admin, contact an admin for more information",
            );
          case "pending":
            setMessage(
              "Your store request is still under review and pending, please wait for an admin to approve your store",
            );
            break;
          default:
            break;
    ...
    return (
      <div className='min-h-[80vh] mx-6 flex items-center justify-center  text-slate-400'>
        <h1 className='text-2xl sm:text-4xl font-semibold'>
          Please <span className='text-slate-500'> Login to Continue </span>
        </h1>
      </div>
    );
  }
